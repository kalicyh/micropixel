...
`Result<IButtonPage>`。`Write(rom, offset, data, password, length)` 写入并回读校验；DS1977 每次写入 64 字节，
DS1991 可在单个 SubKey 范围内写入最多 48 字节。读写均不能跨页/SubKey。
服务调用错误使用 `Result`，器件/总线结果使用 `IButtonStatus`。
