...
Metalio-Claw4 的 DS2484 / DS1977 工位 App。自动检测设备，尝试 Alpha/Beta 密钥，失败时允许手动输入。
认证后默认读取并显示 `0x0000–0x0097`；“完整读取”会继续读取至 DS1977 末尾 `0x7FFF`。
用户可手动触发本地镜像相似度匹配，或选择镜像并逐页写入校验。
...
3. 点“完整读取”会从 `0x0098` 继续读到 `0x7FFF`，然后可按 64 字节分页查看全部 32 KiB。
4. 点“匹配镜像”后，App 对已读取范围与本地镜像按相同字节比例评分，只显示相似度最高的一项。
5. 点“选择数据”打开镜像列表并选择镜像。确认页只提供“完整写入”，写入全部 4 KB，以保留尾部辅助数据。
   每次只写入单颗 DS1977，目标 ROM 在每页写入前复核；写入通过 Scratchpad CRC、设备 ACK 和回读比对。成功后列表短暂标绿，切换页面或列表分页后清除标记。
6. Alpha/Beta 自动认证失败时输入 16 位十六进制值（8 字节密钥）；密钥只保存在本次 App 运行内存中。

Alpha/Beta 规则按 DS1977 ROM 生成读取/写入密码。DS1977 读取保留 3000 µs 强上拉；写入使用 25 ms 强上拉并逐页验证。
